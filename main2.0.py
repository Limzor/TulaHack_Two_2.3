...
def is_open_during(time_range, open_time, close_time):
    ...
    if close_time < open_time:  # В случае если время пересекает полночь
        if start_time < open_time:
            start_time = datetime.combine(datetime.today(), start_time) + timedelta(days=1)
...
def handle_time_range(message):
    ...
    try:
        ...
        cursor.execute('SELECT name, open_time, close_time, work_days, type, who, Shirota, Dolgota, WastedTime FROM attractions')
        available_attractions = []
        for row in cursor.fetchall():
            name, open_time, close_time, work_days, _type, who, shirota, dolgota, wasted_time = row
            if is_open_during(user_time_range, open_time, close_time):
                available_attractions.append((name, open_time, close_time, work_days, _type, wasted_time, shirota, dolgota))
        
        # Сортировка по широте и долготе
        available_attractions.sort(key=lambda x: (x[6], x[7]))

        if available_attractions:
            response_lines = []
            for idx, attraction in enumerate(available_attractions, start=1):
                name, open_time, close_time, work_days, _type, wasted_time, _, _ = attraction
                response_lines.append(
                    f"{idx}. {name}\n"
                    f"   Время работы: {open_time} - {close_time}\n"
                    f"   Рабочие дни: {work_days}\n"
                    f"   Тип: {_type}\n"
                    f"   Время на посещение: {wasted_time}\n"
                )
